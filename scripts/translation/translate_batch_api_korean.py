#!/usr/bin/env python3
"""
Translate sentences to Korean using Gemini Batch API for 50% cost reduction.
...
SYSTEM_PROMPT = """What's the most natural spoken casual Korean way to express the following? Output only the Korean sentence using Hangul.

Return ONLY a JSON array of translations in the same order as input.
Example input: ["Hello", "How are you?"]
Example output: ["안녕하세요", "어떻게 지내세요?"]"""
...
def create_batch_request(sentences: list[dict], start_idx: int) -> dict:
    ...
    prompt = f"Translate these English sentences to natural spoken Korean:\n{json.dumps(english_texts, ensure_ascii=False)}"
...
def process_batch_results(client, job, sentences: list[dict], start_idx: int, output_file: Path):
    ...
    for offset in sorted_offsets:
        ...
        for sent, new_ko in zip(batch_sentences, translations):
            results.append({
                "id": sent["id"],
                "en": sent["en"],
                "ko": new_ko
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Batch API translation to Korean")
    ...
    output_dir = data_dir / "translated_korean"
    ...
    job = submit_batch_job(client, batch_file, f"duoko-translation-{start_idx}")
