#!/usr/bin/env python3
"""
Translate remaining sentences using Gemini Batch API for 50% cost reduction.
Continues from where translate_full_dataset.py left off.
"""
...
# Configuration
MODEL_NAME = "gemini-2.5-flash"  # Batch API uses this model name format
REQUESTS_PER_BATCH = 1000  # Batch multiple sentences per request for efficiency
SENTENCES_PER_REQUEST = 50  # How many sentences to include in each GenerateContentRequest
...
def check_job_status(client, job_name: str):
    ...
    # Show all available job attributes for debugging
    ...
    # Check for progress info (if available)
...
def download_batch_results(client, file_name: str) -> str:
    ...
    # For batch result files, we need to use the download endpoint directly
    # The file_name is like "files/batch-xxxxx"
    # We need to call the REST API directly
    base_url = "https://generativelanguage.googleapis.com/v1beta"

    # Try to download via the files API media endpoint
    ...
    try:
        ...
        # Alternative: try without :download suffix
...
def process_batch_results(client, job, sentences: list[dict], start_idx: int, output_file: Path):
    ...
    if hasattr(job, 'dest') and job.dest:
        if hasattr(job.dest, 'inlined_responses') and job.dest.inlined_responses:
            ...
            # Download and parse file results (JSONL format)
    ...
    # Process responses - USE THE KEY TO ORDER THEM CORRECTLY
    # Key format: "batch-{start}-{end}" where start/end are offsets within remaining sentences
    keyed_translations = {}  # offset -> list of translations
    errors = 0

    for resp in responses:
        # Get the key to determine which batch this is
        key = None
        if hasattr(resp, 'key'):
            key = resp.key
        ...
        # Parse key to get offset: "batch-0-50" -> offset 0
        try:
            parts = key.split('-')
            offset = int(parts[1])
        except:
    ...
    # Now build results in correct order
    results = []
    ...
    for offset in sorted_offsets:
        ...
        for sent, new_ja in zip(batch_sentences, translations):
            results.append({
                "id": sent["id"],
                "en": sent["en"],
                "ja": new_ja
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Batch API translation for Tatoeba dataset")
    ...
    if args.status or args.wait:
        ...
        if state in ('JOB_STATE_SUCCEEDED', 'SUCCEEDED'):
            # Load sentences for processing
            all_sentences = load_deduplicated_sentences(data_dir)
            with open(job_files[-1], "r") as f:
                job_info = json.load(f)
    ...
    # Submit batch job
    job = submit_batch_job(client, batch_file, f"duojp-translation-{start_idx}")
