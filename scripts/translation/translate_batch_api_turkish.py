#!/usr/bin/env python3
"""
Translate sentences to Turkish using Gemini Batch API for 50% cost reduction.
...
SYSTEM_PROMPT = """What's the most natural spoken casual Turkish way to express the following? Output only the Turkish sentence.

Return ONLY a JSON array of translations in the same order as input.
Example input: ["Hello", "How are you?"]
Example output: ["Merhaba", "Nasılsın?"]"""
...
def create_batch_request(sentences: list[dict], start_idx: int) -> dict:
    ...
    prompt = f"Translate these English sentences to natural spoken Turkish:\n{json.dumps(english_texts, ensure_ascii=False)}"
...
def process_batch_results(client, job, sentences: list[dict], start_idx: int, output_file: Path):
    ...
    for offset in sorted_offsets:
        ...
        for sent, new_tr in zip(batch_sentences, translations):
            results.append({
                "id": sent["id"],
                "en": sent["en"],
                "tr": new_tr
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Batch API translation to Turkish")
    ...
    output_dir = data_dir / "translated_turkish"
    ...
    job = submit_batch_job(client, batch_file, f"duotr-translation-{start_idx}")
